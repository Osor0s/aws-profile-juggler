...
AWS_ACCESS_KEY_ID_ALIASES = ['aws access key id', 'aws.access.key.id', 'aws_access_key_id', 'aws-access-key-id', 'aws\-access\-key\-id', 'aws\_access\_key\_id', 'aws\ access\ key\ id', 'aws access key', 'aws.access.key', 'aws_access_key', 'aws-access-key', 'aws\-access\-key', 'aws\_access\_key', 'aws\ access\ key', 'aws key', 'aws.key', 'aws_key', 'aws-key', 'aws\-key', 'aws\_key', 'aws\ key', 'access key', 'access.key', 'access_key', 'access-key', 'access\-key', 'access\_key', 'access\ key', 'aws key id', 'aws.key.id', 'aws_key_id', 'aws-key-id', 'aws\-key\-id', 'aws\_key\_id', 'aws\ key\ id', 'access key id', 'access.key.id', 'access_key_id', 'access-key-id', 'access\-key\-id', 'access\_key\_id', 'access\ key\ id']
...
def write_credentials(credentials, file, profile, overwrite):
    ...
    with open(file,'r+') as aws_credentials_file:
        ...
        if existing_entry_start == -1:
            ...
            if not overwrite:
                logging.warning('Profile already exists! Either choose a different profile name with the option "-p" OR use "-o" to overwrite the existing profile')
                return
            else:
